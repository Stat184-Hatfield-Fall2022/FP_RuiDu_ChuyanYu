...
DisneyBox <- read_excel("/Users/yuchuyan/Desktop/DisneyBox.xlsx")  #input/read  data 


disney <- read.csv(#input/read  data 
  file="/Users/yuchuyan/Desktop/disney_plus_projects.csv",
  header=TRUE,
  sep=","
)

disney2 <- read.csv(#input/read  data 
...
Disney6 <- inner_join( #combine the data 
  x = disney,
  y = DisneyBox,
...
disney2 %>%  #first data 
  group_by(release_year) %>% #clean data 
  summarize(n = n()) %>%
...
Disney6 %>%  
 filter(genre.y %in% c("Adventure", "Thriller/Suspense", "Action", "Romantic Comedy", "Drama")) %>%  #clean data
 ggplot() +
  aes(x = genre.y, y = rating) +
  geom_boxplot(fill = "#B22222") +
  theme_gray()+
  labs(
    x = "Type of movie",  #graph label 
    y = "Rating",
...
Disney6 %>%
 filter(genre.y %in% c("Comedy", "Musical", "Western", "Horror", "Documentary")) %>% #clean data 
 ggplot() +
  aes(x = genre.y, y = rating) +
  geom_boxplot(fill = "#B22222") +
  theme_gray()+
  labs( 
    x = "Type of movie", #data label 
    y = "Rating",
    title = "Rating by different Film Categore ",
  ) 
...
data= Disney6 #Film Categore and Rating separate relation for Movie tabel
disneystat <- describeBy(
  ...
  select("Subject matter" = group1, Rating = vars,  #determine want we want to show 
...
DisneyY <-disney %>%
  filter(rating>0)   #clean data

source("https://raw.github.com/neilhatfield/STAT461/master/rScripts/shadowgram.R")  #use shadow graph function to generate graph  #use online modle to generate graph 
shadowgram(  #shadow graph function 
...
# show all the code at the end of pdf
```
